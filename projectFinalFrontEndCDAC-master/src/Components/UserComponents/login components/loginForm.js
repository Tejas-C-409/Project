...
export function Login() {
  ...
  let validate = () => {
    if (username === "" && password === "") {
      Swal.fire({ text: "Empty fields are not allowed", icon: "error" });
    } else {
      login();
    }
  };
  ...
  let login = () => {
    ...
    axios.post(`${baseurl}/login`, user).then(
      (response) => {
        ...
        console.log(urole);
        if (urole === "CONTRACTOR") {
          axios.get(`${baseurl}/user/getContractorByUSerId/${uid}`).then(
            (response) => {
          ...
          axios.get(`${baseurl}/user/getLabourByUSerId/${uid}`).then(
            (response) => {
              console.log(response.data);
      ...
      (error) => {
        ...
        Swal.fire({
          title: "Oops",
          text: " user might not exists in our system",
          icon: "error",
        });
      }
    );
