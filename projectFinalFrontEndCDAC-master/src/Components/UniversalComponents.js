...
export default function ShowCReview() {
  ...
  let getContractors = () => {
    ...
    if (role1 == "USER") {
      ...
      axios.get(`${baseurl}/user/getreviewbyuserid/${id}`).then(
        (response) => {
      ...
      let id = l.labourId;
      axios.get(`${baseurl}/labour/getallreviewbylabourId/${id}`).then(
        (response) => {
          setreviewList(response.data);
          toast.success("Contracotr Data Loaded Successfully");
        },
      ...
      axios.get(`${baseurl}/labour/getallreviewbycontractorid/${id}`).then(
        (response) => {
          setreviewList(response.data);
          toast.success("Contracotr Data Loaded Successfully");
        },
  ...
  return (
    <div>
      ...
      {reviewList.map((userset) => {
        return (
          <Card
            key="{userset.reviewId}"
            color="info"
            inverse
            className="m-1 justify-content-center text-center"
          >
            <CardHeader>
